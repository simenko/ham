...
Actor.prototype.defaultListener = function (msg)  {
  console.log(this.id() + ': Ooops! ' + msg._id + ', ' + msg.channel);
}

/**
 * Recieves message, adds it to the queue and dispatches 
...
Actor.prototype.checkQueue = function()  {
  return this._queue;
}
...
Actor.prototype.sub = function (channel, listener)  {
  ...
  if (this.hub.subscribe (channel, this.id(), this))  {
    return true;
  }
  return false;
}
...
Actor.prototype.pub = function (channel, body, options) {
  ...
  return this.hub.publish (msg, this.id(), this);
}
